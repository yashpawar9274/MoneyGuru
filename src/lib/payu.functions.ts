...
function payuCreds() {
  ...
  if (!key || !salt)
    throw new Error("PayU is not configured yet. Add PAYU_MERCHANT_KEY and PAYU_MERCHANT_SALT.");
  return {
    key,
    salt,
    env,
    action:
      env === "production" ? "https://secure.payu.in/_payment" : "https://test.payu.in/_payment",
  };
}
...
export const createPayUCheckout = createServerFn({ method: "POST" })
  ...
  .inputValidator(
    (data: { plan: PaidPlan; returnUrl: string; phone?: string; firstName?: string }) => {
      if (!["weekly", "pro", "lifetime"].includes(data.plan)) throw new Error("Invalid plan");
      if (!/^https:\/\//.test(data.returnUrl) && !/^http:\/\/localhost/.test(data.returnUrl))
  ...
  .handler(async ({ data, context }) => {
    ...
    const firstname = (
      data.firstName || String(context.claims?.user_metadata?.full_name || "MoneyGuru User")
    )
      .trim()
      .slice(0, 60);
    ...
    const productinfo =
      data.plan === "lifetime"
        ? "MoneyGuruAI Lifetime"
        : data.plan === "weekly"
          ? "MoneyGuruAI Starter 7 Days"
          : "MoneyGuruAI Pro 30 Days";
    ...
    const { error } = await supabaseAdmin.from("payments").insert({
      ...
      order_id: txnid,
      plan: data.plan,
      amount_inr: PLAN_PRICE_INR[data.plan],
      status: "created",
      provider: "payu",
    });
...
export const confirmPayUCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row } = await supabaseAdmin
      .from("payments")
      .select("user_id,plan,status,amount_inr,provider")
      .eq("order_id", data.txnid)
      .maybeSingle();
    if (!row || row.user_id !== context.userId || row.provider !== "payu")
    ...
    const body = new URLSearchParams({
      ...
      command,
      var1: data.txnid,
      hash: sha512(`${key}|${command}|${data.txnid}|${salt}`),
    });
    const url =
      env === "production"
    ...
    const transactionDetails =
      payload && typeof payload === "object" && "transaction_details" in payload
    ...
    const paid = ["success", "captured"].includes(
      String(tx.status || tx.unmappedstatus || "").toLowerCase(),
    );
