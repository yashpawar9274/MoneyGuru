...
export function ledger(d: Debt): LedgerItem[] {
  ...
  return [...given, ...paid].sort((a, b) => {
    ...
    return difference || b.id.localeCompare(a.id);
  });
}

/** Total actually lent/borrowed from the transaction timeline, never only the last entry. */
export function givenTotal(d: Debt) {
  return ledger(d)
