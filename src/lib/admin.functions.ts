...
export const getAdminDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<AdminDashboardData> => {
    ...
    const [profilesResult, paymentsResult, subscriptionsResult, webhooksResult] = await Promise.all([
      supabaseAdmin.from("profiles").select("id,email,full_name,created_at").order("created_at", { ascending: false }).limit(250),
      supabaseAdmin.from("payments").select("id,user_id,order_id,amount_inr,plan,status,provider,created_at,updated_at").order("created_at", { ascending: false }).limit(250),
      supabaseAdmin.from("subscriptions").select("user_id,plan,status,price_inr,trial_ends_at,current_period_end,updated_at").order("updated_at", { ascending: false }).limit(250),
      supabaseAdmin.from("webhook_logs").select("id,provider,event_type,order_id,status,signature_valid,http_status,message,created_at").order("created_at", { ascending: false }).limit(250),
    ]);

    const firstError = profilesResult.error || paymentsResult.error || subscriptionsResult.error || webhooksResult.error;
